...
export default function MethodDetail({
  valuation, selected, currency, onClose,
}: { valuation: Valuation; selected: string | null; currency: string; onClose: () => void }) {
  useEffect(() => {
  ...
  if (selected === "blended") {
    ...
    return (
      <Scrim onClose={onClose}>
        ...
        <div className="steps">
          {used.map((m) => (
            <div className="step" key={m.key}><span>{m.label} midpoint</span><span className="step-val">{fmtUSD(m.mid)}</span></div>
          ))}
  ...
  return (
    <Scrim onClose={onClose}>
      ...
      {!m.available && (
        <p className="muted" style={{ margin: "12px 0" }}>{m.unavailableReason}</p>
      )}
      ...
      <div className="sources">
        ...
        {m.sources.length === 0 && <p className="muted small">No filing-level source recorded for this method.</p>}
        {m.sources.map((s, i) => (
          <div className="source-link" key={i}>
            ...
            {s.url && <a href={s.url} target="_blank" rel="noreferrer">view filing ↗</a>}
          </div>
        ))}
      </div>
    </Scrim>
